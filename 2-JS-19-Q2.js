//? Question-2:Write a code that, upon clicking a button labeled "Place Bid" changes its text to "Added to Cart" and disables the button (you can modify the style if necessary).
// hint: watch help.mp4
//!Answer:
let elementIs = document.querySelector(".cards__card__footer__btn button p");
let shopIcon = document.querySelector(".ri-handbag-line");
elementIs.addEventListener("click", () => {
  elementIs.innerText = "Added to Cart";
  shopIcon.style.display = "none";
});
