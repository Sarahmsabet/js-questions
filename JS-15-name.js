...
//! Answer:
let enterName = prompt("Enter your character please", "").toUpperCase();
let mainFunc = () => {
  while (!(enterName >= "A" && enterName <= "Z") || !(enterName.length == 1)) {
    if (!(enterName >= "A" && enterName <= "Z")) {
      alert("enter only alphabets");
      enterName = prompt("Enter your character please", "").toUpperCase();
    }
  ...
  if (enterName >= "A" && enterName <= "Z" && enterName.length == 1) {
    return names.filter((enName) => {
      return enName.startsWith(enterName);
    });
  }
};
