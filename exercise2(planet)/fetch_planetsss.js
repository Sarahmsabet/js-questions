...
const hThreeElement = document.querySelector("h3");
const divElement = document.querySelector("#destination");
let counter = 0;
...
//     );
//     const arrayOfPlanet = await planet.json();
//     hOneElement.innerText = `Destination: ${arrayOfPlanet[0].distance}`;
//     hThreeElement.innerText = `Planet: ${arrayOfPlanet[0].name}`;
//     divElement.addEventListener("click", () => {
//       counter += 1;
//       if (counter === 6) {
//         counter = 0;
//         hOneElement.innerText = `Destination: ${arrayOfPlanet[counter].distance}`;
//         hThreeElement.innerText = `Planet: ${arrayOfPlanet[counter].name}`;
//       } else {
//         hOneElement.innerText = `Destination: ${arrayOfPlanet[counter].distance}`;
//         hThreeElement.innerText = `Planet: ${arrayOfPlanet[counter].name}`;
//       }
//     });
//   } catch (error) {
//     console.log("gashram nabud nagard nist");
//   }
// };
...
const planetList = async () => {
  try {
    const planet = await axios.get(
      ...
      const arrayOfPlanet = planet.data;
      hOneElement.innerText = `Destination: ${arrayOfPlanet[0].distance}`;
      hThreeElement.innerText = `Planet: ${arrayOfPlanet[0].name}`;
      divElement.addEventListener("click", () => {
        counter += 1;
      ...
      console.log("gashtam nabud nagard nist");
    }
  };
